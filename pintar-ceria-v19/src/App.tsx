...
import initialData from './data/initialQuestions.json';
// const initialData = {};
...
export default function App() {
  ...
  const loadMapelCounts = async () => {
    ...
    for (const m of MAPEL_LIST) {
      let data = await localforage.getItem<MapelData>(m.id);
      // Fallback to initialData if not in localforage
      if (!data && (initialData as any)[m.id]) {
        data = { questions: (initialData as any)[m.id] } as any;
      }
  ...
  const startQuiz = async (mapelId: string) => {
    let data = await localforage.getItem<MapelData>(mapelId);
    // Fallback to initialData
    if (!data && (initialData as any)[mapelId]) {
      data = { questions: (initialData as any)[mapelId] } as any;
    }
  ...
  const handleExportData = async () => {
    const exportData: Record<string, Question[]> = {};
    for (const m of MAPEL_LIST) {
      const data = await localforage.getItem<MapelData>(m.id);
      if (data && data.questions && data.questions.length > 0) {
        exportData[m.id] = data.questions;
      }
    }
    
    // Merge with existing initialData if needed, but prioritize localforage
    // Actually, we just want to export what we have so the user can replace initialData
    ...
    a.href = url;
    a.download = 'initialQuestions.json';
  ...
  const generateQuestions = async () => {
    ...
    try {
        ...
        try {
          const result = await ai.models.generateContent({
            ...
            config: {
              ...
              INSTRUKSI GAMBAR/VISUAL:
              1. Jika soal di 'Contoh Soal (SAS)' memiliki gambar, atau jika soal membutuhkan ilustrasi (terutama Matematika/IPA), WAJIB sertakan field "imageKeyword".
              2. "imageKeyword" harus berupa 1-3 kata kunci dalam BAHASA INGGRIS yang spesifik untuk mencari gambar tersebut (contoh: "volcano eruption", "right triangle geometry", "indonesian flag", "photosynthesis process").
              3. Sertakan juga field "illustration" berisi deskripsi teks bahasa Indonesia untuk membantu siswa jika gambar tidak muncul.
              
              Setiap soal butuh field: "materi", "type", "format", "question", "options", "answer", "explanation", "conversation", "illustration", "imageKeyword".
      ...
      const parsedNewQuestions = Array.isArray(newQuestions) ? newQuestions : (newQuestions.questions || []);

      // Load existing questions
      const existingData = await localforage.getItem<MapelData>(adminMapelId);
      let existingQuestions = existingData?.questions || [];
      
      // Fallback to initialData if localforage is empty
      if (existingQuestions.length === 0 && (initialData as any)[adminMapelId]) {
        existingQuestions = (initialData as any)[adminMapelId];
      }

      // Merge Logic: Replace duplicates based on question text, append new ones
      const mergedQuestions = [...existingQuestions];
      
      parsedNewQuestions.forEach((newQ: Question) => {
        // Normalize text for comparison (remove extra spaces, lowercase)
        const normalize = (s: string) => s.toLowerCase().replace(/\s+/g, ' ').trim();
        const newQText = normalize(newQ.question);
        
        const existingIndex = mergedQuestions.findIndex(eq => normalize(eq.question) === newQText);
        
        if (existingIndex !== -1) {
          // Replace existing question with the new one (updated version)
          mergedQuestions[existingIndex] = newQ;
        } else {
          // Append new question
  ...
  return (
    <div className="min-h-screen bg-blue-50 text-slate-800 font-sans selection:bg-pink-300 selection:text-white pb-10"
         style={{
           backgroundImage: `url("data:image/svg+xml,%3Csvg width='300' height='300' viewBox='0 0 300 300' xmlns='http://www.w3.org/2000/svg'%3E%3Cg opacity='0.5'%3E%3Ctext x='40' y='80' font-size='50'%3E👮%3C/text%3E%3Ctext x='200' y='80' font-size='50'%3E🧑‍⚕️%3C/text%3E%3Ctext x='40' y='240' font-size='50'%3E🧑‍🏫%3C/text%3E%3Ctext x='200' y='240' font-size='50'%3E🧑‍✈️%3C/text%3E%3Ctext x='120' y='160' font-size='50'%3E🧑‍🚒%3C/text%3E%3C/g%3E%3C/svg%3E")`,
           backgroundSize: '300px 300px'
         }}
    >
      {/* Navbar */}
      <nav className="bg-white/90 backdrop-blur-md shadow-sm p-3 sticky top-0 z-50 border-b-4 border-yellow-300">
            ...
            >
              {!isStudentLoggedIn ? (
                ...
                <>
                  ...
                  <div className="bg-white rounded-3xl p-6 shadow-md border-4 border-yellow-300 mb-8 relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-yellow-300 text-yellow-900 text-xs font-bold px-3 py-1 rounded-bl-xl">
                      PENTING
                    </div>
                    <label className="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Jumlah Soal Kuis</label>
                    <input 
                      type="number" 
                    ...
                    <p className="text-center text-xs text-gray-500 mt-2 font-medium">
                      * Maksimal soal sesuai dengan jumlah soal per matapelajaran yang tertera di setiap kartu.
                    </p>
                  </div>
          ...
          {mode === 'quiz' && activeQuiz && (
            ...
            >
              ...
              <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-xl border-4 border-gray-100 relative overflow-hidden">
                ...
                {(activeQuiz.questions[activeQuiz.currentIndex].illustration || activeQuiz.questions[activeQuiz.currentIndex].imageKeyword) && (
                  <div className="mb-8 bg-gray-50 p-6 rounded-3xl border-2 border-gray-100 flex flex-col items-center relative z-10">
                    <div className="w-full max-w-lg h-64 bg-white rounded-2xl flex items-center justify-center mb-4 overflow-hidden relative shadow-sm border border-gray-200">
                       <img 
                         src={activeQuiz.questions[activeQuiz.currentIndex].imageKeyword 
                           ? `https://picsum.photos/seed/${activeQuiz.questions[activeQuiz.currentIndex].imageKeyword}/800/400`
                           : `https://placehold.co/800x400/f1f5f9/475569?text=Ilustrasi+Soal&font=montserrat`}
                         alt="Ilustrasi Soal" 
                         className="w-full h-full object-cover opacity-90 hover:scale-105 transition-transform duration-500"
                         referrerPolicy="no-referrer"
                       />
                       {!activeQuiz.questions[activeQuiz.currentIndex].imageKeyword && (
              ...
              <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-md">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-gray-100 pb-6">
                  ...
                  <button 
                    ...
                    className="bg-white border-2 border-gray-200 hover:bg-gray-50 text-gray-600 font-bold px-4 py-3 rounded-xl flex items-center gap-2 text-sm transition-all shadow-sm"
                    title="Download semua soal yang sudah dibuat agar bisa disimpan permanen di kode."
                  >
                    <Download className="w-4 h-4" />
                    Simpan Data Soal (JSON)
                  </button>
                </div>
                  ...
                  {adminMapelId && (
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* Referensi Utama */}
                        <div className="bg-blue-50/80 p-6 rounded-3xl border-2 border-blue-200 hover:border-blue-400 transition-all">
                          ...
                          <input key={adminMapelId + '-materi'} type="file" multiple accept=".txt,.pdf,.docx" onChange={(e) => setAdminMateriFiles(e.target.files)} className="block w-full text-sm text-gray-600 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer shadow-sm transition-all" />
                          {adminFiles[adminMapelId]?.materi?.length > 0 && (
                            <div className="mt-3 bg-white/80 p-3 rounded-xl border border-blue-100 text-xs text-blue-800">
                              <span className="font-bold block mb-1">File Tersimpan:</span>
                              <ul className="list-disc list-inside space-y-1">
                                {adminFiles[adminMapelId].materi.map((f, i) => <li key={i} className="truncate">{f}</li>)}
                              </ul>
                            </div>
                          )}
                        </div>
                        ...
                        {/* Contoh Soal */}
                        <div className="bg-yellow-50/80 p-6 rounded-3xl border-2 border-yellow-200 hover:border-yellow-400 transition-all">
                          ...
                          <input key={adminMapelId + '-sas'} type="file" multiple accept=".txt,.pdf,.docx" onChange={(e) => setAdminSasFiles(e.target.files)} className="block w-full text-sm text-gray-600 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-yellow-600 file:text-white hover:file:bg-yellow-700 cursor-pointer shadow-sm transition-all" />
                          {adminFiles[adminMapelId]?.sas?.length > 0 && (
                            <div className="mt-3 bg-white/80 p-3 rounded-xl border border-yellow-100 text-xs text-yellow-800">
                              <span className="font-bold block mb-1">File Tersimpan:</span>
                              <ul className="list-disc list-inside space-y-1">
                                {adminFiles[adminMapelId].sas.map((f, i) => <li key={i} className="truncate">{f}</li>)}
                              </ul>
                            </div>
                          )}
                        </div>
                        ...
                        {/* Kisi-kisi */}
                        <div className="bg-green-50/80 p-6 rounded-3xl border-2 border-green-200 hover:border-green-400 transition-all">
                          ...
                          <input key={adminMapelId + '-kisi'} type="file" accept=".txt,.pdf,.docx" onChange={(e) => setAdminKisiFile(e.target.files && e.target.files[0] ? e.target.files[0] : null)} className="block w-full text-sm text-gray-600 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-green-600 file:text-white hover:file:bg-green-700 cursor-pointer shadow-sm transition-all" />
                          {adminFiles[adminMapelId]?.kisi && (
                            <div className="mt-3 bg-white/80 p-3 rounded-xl border border-green-100 text-xs text-green-800">
                              <span className="font-bold block mb-1">File Tersimpan:</span>
                              <div className="truncate">• {adminFiles[adminMapelId].kisi}</div>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 p-6 rounded-3xl border-2 border-gray-200 flex flex-col md:flex-row gap-6 items-end shadow-sm">
                        <div className="w-full md:w-1/3 space-y-4">
                          <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Soal Tersedia</label>
                            <div className="text-3xl font-black text-gray-800">
                              {mapelQuestionsCount[adminMapelId] || 0} <span className="text-sm font-normal text-gray-500">soal</span>
                            </div>
                          </div>
                          <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Buat Soal Baru</label>
                            <input 
                              type="number" 
                        ...
                        <div className="w-full md:w-2/3 space-y-3">
                          ...
                          {/* Fitur Mengurangi Soal */}
                          <div className="flex gap-2">
                             <button
                                onClick={async () => {
                                  if (!adminMapelId) return;
                                  if (!confirm('Hapus 5 soal terakhir?')) return;
                                  const data = await localforage.getItem<MapelData>(adminMapelId);
                                  if (data && data.questions) {
                                    const newQ = data.questions.slice(0, Math.max(0, data.questions.length - 5));
                                    await localforage.setItem(adminMapelId, { ...data, questions: newQ });
                                    loadMapelCounts();
                                    showModal('Berhasil', '5 Soal terakhir dihapus.', '🗑️');
                                  }
                                }}
                                className="flex-1 bg-red-100 hover:bg-red-200 text-red-600 font-bold py-3 px-4 rounded-xl text-sm transition-all"
                             >
                               Hapus 5 Terakhir
                             </button>
                             <button
                                onClick={async () => {
                                  if (!adminMapelId) return;
                                  if (!confirm('Yakin hapus SEMUA soal mapel ini?')) return;
                                  const data = await localforage.getItem<MapelData>(adminMapelId);
                                  await localforage.setItem(adminMapelId, { ...data, questions: [] });
                                  loadMapelCounts();
                                  showModal('Reset Berhasil', 'Semua soal dihapus.', '🗑️');
                                }}
                                className="flex-1 bg-red-100 hover:bg-red-200 text-red-600 font-bold py-3 px-4 rounded-xl text-sm transition-all"
                             >
                               Hapus Semua
      ...
      <footer className="text-center text-gray-400 text-sm mt-8 pb-4">
        &copy; 2026|RDG Pintar Ceria - v1.9 (Admin Refresh & Reduce Qs)
      </footer>
    </div>
  );
}
